...
url_webstite = f'{root}/movies_letter-A'
...
## Consultar links de paginacion

links = []

for page in range(1, int(last_page)+1)[:5]: ## Itera hasta las primeras 5
    result = requests.get(f'{url_webstite}?page={page}')  ## Obtiene la pagina
    content = result.text  ## Obtener texto de el resultado
    soup = BeautifulSoup(content, 'lxml')  ## Contenido + Parser
